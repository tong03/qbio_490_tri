...
## Create columns in rna_clinical to represent the filtered df above

# both alcohol and cigs column
rna_clinical$both_alc_cig <- ifelse(rna_clinical$patient %in% both$patient, "Yes", "No")

# Alcohol only column
rna_clinical$alc_only <- ifelse(rna_clinical$patient %in% alc_only$patient, "Yes", "No")

# Cigs only column
rna_clinical$cig_only <- ifelse(rna_clinical$patient %in% cig_only$patient, "Yes", "No")

# Usage column to classify patients into 4 categories
rna_clinical$usage <- ifelse(rna_clinical$patient %in% both$patient, "Both", 
                             ifelse(rna_clinical$patient %in% alc_only$patient, "Alcohol", 
                                    ifelse(rna_clinical$patient %in% 
...
both_A_mask <- ifelse(rna_clinical$usage == "Both" |
                           rna_clinical$usage == "Alcohol", T, F)
both_A_df = rna_clinical[both_A_mask,]
...
```{r}
both_C_mask <- ifelse(rna_clinical$usage == "Cigarettes" |
                           rna_clinical$usage == "Both", T, F)
both_C_df = rna_clinical[both_C_mask,]
...
Survival Plot of Alc-only vs. Cigs-only user
```{r}
# Select only for alc-only and cigs-only patients
only_mask <- ifelse(rna_clinical$usage == "Cigarettes" |
                           rna_clinical$usage == "Alcohol", T, F)
only_df = rna_clinical[only_mask,]

# Create survival_time column in both_df to make survival plot
only_df$survival_time <- ifelse(only_df$days_to_death == "NA",
                                only_df$days_to_last_follow_up,
                                only_df$days_to_death)
# unique(both_df$survival_time) indicates there are still NA values
only_NA_mask <- ifelse(only_df$survival_time == "NA", F, T)
only_df[only_NA_mask,]

# Create survival event column (T/F) for survival plot --> T is dead, F is alive
only_df$death_event <- ifelse(only_df$vital_status == "Alive", F, T)

survival_object <- Surv(time = as.numeric(only_df$survival_time), event = only_df$death_event)
fit_object <- survfit(survival_object ~ usage, data = only_df)
...
#                              design = ~gender + race + vital_status + definition +
#                                chemo + immuno + vaccine + moleculo + radiation + usage)

dds <- DESeqDataSetFromMatrix(countData = rm_low_counts,
                             colData = rna_clinical1,
                             design = ~gender + race + vital_status + definition +
                               chemo + radiation + usage)
dds_obj <- DESeq(dds)
...
```{r}
par(mar=c(1,1,1,1))
ev1 <- EnhancedVolcano(sig_results,
    lab = sig_results$`Gene Name`,
    x = 'log2FoldChange',
    y = 'pvalue',
